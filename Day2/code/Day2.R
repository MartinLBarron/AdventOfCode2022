...
score_game <-function(opponent, self){
  return (case_when(opponent=="Rock" & self=="Rock" ~ 3,
...
score_pick <-function(self){
  return(case_when(self == "Rock" ~ 1,
                   self == "Paper" ~ 2,
                   self == "Scissors" ~3))
}
...
df <- read_delim("Day2/data/input.txt", col_names=F, delim=" ") %>%
  mutate(opponent=fct_recode(X1,"Rock" = "A",
         ...
         self = fct_recode(X2, "Rock" = "X",
                           "Paper" = "Y",
                           "Scissors" = "Z"),
         score_pick=score_pick(self),
         score_game=score_game(opponent, self),
